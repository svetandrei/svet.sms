...
if ($request->get('via_ajax') === 'Y')
{
	...
	try
	{
		$params = $signer->unsign($request->get('signedParamsString'), 'sms.notifications');
	...
	try
	{
		$template = $signer->unsign($request->get('template'), 'sms.notifications');
	}
	catch (Exception $e)
	{
		$template = '.default';
